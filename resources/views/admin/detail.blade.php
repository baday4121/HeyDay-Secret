...
@section('title', 'Admin Message Detail')

@section('content')
    <div class="w-full max-w-md px-2 sm:px-0">
        <a href="{{ route('dashboard') }}" class="px-3 py-1.5 bg-white border border-blue-300 rounded-lg text-blue-600 hover:bg-blue-50 mb-4 inline-block text-xs font-semibold shadow-sm transition">
            Back to Dashboard
        </a>
        ...
        <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200 mb-6 relative overflow-hidden">
            ...
            <p class="text-gray-700 text-sm mb-4 break-words whitespace-pre-line font-semibold">{{ $message->content }}</p>

            <div class="bg-gray-50 p-3 rounded-lg border border-gray-200 text-xs text-gray-600 font-mono leading-relaxed">
                <div class="font-bold text-gray-800 mb-1">Tracking Info:</div>
                <div><span class="text-gray-400">Location :</span> {{ $message->location ?? 'N/A' }}</div>
                <div><span class="text-gray-400">IP Address :</span> {{ $message->ip_address ?? 'N/A' }}</div>
                <div><span class="text-gray-400">Device :</span> {{ $message->user_agent ?? 'N/A' }}</div>
            </div>
        ...
        <div class="space-y-4 mb-8">
            ...
            @forelse($message->replies as $reply)
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 relative overflow-hidden">
                    ...
                    <div class="bg-gray-50 p-3 rounded-lg border border-gray-200 text-xs text-gray-600 font-mono leading-relaxed">
                        <div class="font-bold text-gray-800 mb-1">Tracking Info:</div>
                        <div><span class="text-gray-400">Location :</span> {{ $reply->location ?? 'N/A' }}</div>
                        <div><span class="text-gray-400">IP Address :</span> {{ $reply->ip_address ?? 'N/A' }}</div>
