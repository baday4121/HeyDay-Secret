...
<div class="w-full max-w-3xl">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
        <div class="flex gap-2 flex-wrap">
            <a href="{{ route('dashboard', ['tab' => 'inbox']) }}" class="px-4 py-2 rounded-lg text-xs font-bold transition {{ request('tab', 'inbox') === 'inbox' ? 'bg-blue-600 text-white shadow-sm' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50' }}">
                Inbox
            </a>
            
            <a href="{{ route('dashboard', ['tab' => 'archived-messages']) }}" class="px-4 py-2 rounded-lg text-xs font-bold transition flex items-center gap-2 {{ request('tab') === 'archived-messages' ? 'bg-blue-600 text-white shadow-sm' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50' }}">
                <span>Archived Messages</span>
                
                @if(isset($archivedMessagesCount) && $archivedMessagesCount > 0)
                    <span class="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-extrabold animate-pulse">
                        {{ $archivedMessagesCount }}
                    </span>
                @endif
            </a>

            <a href="{{ route('dashboard', ['tab' => 'archived-replies']) }}" class="px-4 py-2 rounded-lg text-xs font-bold transition flex items-center gap-2 {{ request('tab') === 'archived-replies' ? 'bg-blue-600 text-white shadow-sm' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50' }}">
                <span>Archived Replies</span>
                
                @if(isset($archivedRepliesCount) && $archivedRepliesCount > 0)
                    <span class="bg-red-500 text-white text-[10px] px-1.5 py-0.5 rounded-full font-extrabold animate-pulse">
        ...
        <div class="text-xs text-gray-500 font-medium bg-white px-3 py-2 rounded-lg border border-gray-200 shadow-sm">
            Total Archive: <span class="font-bold text-gray-800">{{ $archivedMessagesCount ?? 0 }} Messages</span> | <span class="font-bold text-gray-800">{{ $archivedRepliesCount ?? 0 }} Replies</span>
        </div>
    </div>
    ...
    @if(request('tab') === 'archived-replies')
        @if(isset($archivedReplies) && $archivedReplies->isEmpty())
        ...
        @else
            <div class="space-y-4">
                @foreach ($archivedReplies as $reply)
                    <div class="bg-white p-5 rounded-xl shadow-sm border-l-4 border-yellow-500 flex flex-col gap-3">
                        <div class="flex justify-between items-start">
                            <div class="flex items-center gap-2">
                                <span class="bg-yellow-100 text-yellow-800 text-[10px] font-bold px-2 py-0.5 rounded">ARCHIVED REPLY</span>
                                <p class="text-gray-400 text-xs font-medium">{{ $reply->created_at->diffForHumans() }}</p>
                            </div>
                        </div>
                        
                        <div class="pl-1">
                            <p class="text-gray-800 text-sm break-words whitespace-pre-line leading-relaxed font-semibold">{{ $reply->content }}</p>
                        </div>

                        @if($reply->message)
                            <div class="bg-gray-50 p-3 rounded-lg border border-gray-100 text-xs text-gray-600">
                                <span class="font-bold text-gray-700">Key Message:</span> 
                                <p class="italic text-gray-500 mt-0.5">"{{ Str::limit($reply->message->content, 100) }}"</p>
                            </div>
                        @endif
                        
                        <div class="mt-2 pt-3 border-t border-gray-100 flex justify-end items-center gap-3">
                            <form action="{{ route('reply.archive', $reply->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="text-yellow-600 hover:text-yellow-800 text-[11px] font-medium transition">
                                    Unarchive
                                </button>
                            </form>

                            @if($reply->message)
                                <a href="{{ route('admin.message.show', $reply->message->id) }}" class="text-blue-500 hover:text-blue-700 hover:underline text-[11px] font-medium transition whitespace-nowrap">
                                    View Details
                                </a>
                            @endif
                        </div>
                        ...
                        <div class="flex justify-between items-start">
                            <div class="flex items-center gap-2">
                                ...
                                @if(isset($msg->replies_count) && $msg->replies_count > 0)
                                    <div class="bg-blue-50 text-blue-600 border border-blue-200 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                        ...
                        <div class="mt-2 pt-3 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            ...
                            <div class="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                                ...
                                <div class="flex items-center gap-3">
                                    <form action="{{ route('message.archive', $msg->id) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="text-yellow-600 hover:text-yellow-800 text-[11px] font-medium transition">
